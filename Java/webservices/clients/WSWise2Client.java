...
import uk.ac.ebi.webservices.WSWise2.*;

public class WSWise2Client {
	...
	System.out.println("Use: java WSWise2Client [options] --email your@email.com --program PROGNAME --database DBNAME --sequence file");
	System.out.println("Use: java WSWise2Client [options] --email your@email.com --program PROGNAME --database DBNAME --sequence db:entry_id");
	System.out.println("");
	...
	Iterator ii = options.getOptions().iterator();
	 ...
	 System.out.println(" --" + opt.getLongOpt() + " : " + opt.getDescription());
	    ...
	    System.out.println("  Usage: java WSWise2Client [options] --program PROGNAME --database DBNAME --sequence file [--outfile string]");
	    ...
	    System.out.println("  Use: java WSWise2Client [options] --program PROGNAME --database DBNAME --sequence file --async");
	    ...
	    System.out.println("");
	    System.out.println("  When done, WSWise2Client --polljob --jobid string [--outfile string]");
	    System.out.println("");
    ...
    private static int checkStatus(String jobid) throws IOException {
	 try {
	    ...
	    WSWise2Service service =  new WSWise2ServiceLocator();
	    WSWise2 wise2 = service.getWSWise2();
	    String result =  wise2.checkStatus(jobid);
	    System.out.println("This is your job id: "+ jobid+ " ( " +result+" )");
	    System.out.println("Use: java WSWise2Client --polljob --jobid "+ jobid + " to see the results");	
	 ...
	 try {
	    ...
	    if (line.hasOption("outformat")) {
	     ...
	     if (line.getOptionValue("outformat")=="xml") {format="toolxml";}
             byte[] resultbytes = wise2.poll(jobid,format);
	     ...
	     if (line.hasOption("stdout")){ 
 	     ...
 	     WSFile[] results = wise2.getResults(jobid);

	     for (int i=0;i<results.length;i++){
		WSFile file = results[i];
		byte[] resultbytes = wise2.poll(jobid,file.getType());
		String result = new String(resultbytes);		  
	...
	return 0;
	 ...
	 try {
	    ...
	    WSWise2Service service =  new WSWise2ServiceLocator();
	    WSWise2 wise2 = service.getWSWise2();
	    String[] results = wise2.getIds(jobid);
	    return results;
 ...
 try {
	...
	input.setType("sequence");
        ...
        fileContent = readFile(new File(line.getOptionValue("sequence")));
	input.setContent(new String(fileContent));
	inputs[0]=input;
...
/*    if (line.hasOption("exp")) params.setExp(Float.parseFloat(line.getOptionValue("exp")));
    if (line.hasOption("scores")) params.setScores(Integer.parseInt(line.getOptionValue("scores"))); 
    if (line.hasOption("numal")) params.setNumal(Integer.parseInt(line.getOptionValue("numal"))); 
    if (line.hasOption("topcombon")) params.setTopcombon(Integer.parseInt(line.getOptionValue("tomcombon"))); 
*/
...
options.addOption(OptionBuilder.withLongOpt("database")
    ...
    try {
      ...
      if (line.hasOption("sequence")||line.hasOption("dbfetch")||line.hasOption("ebifile")) {
	    ...
	    WSWise2Service service =  new WSWise2ServiceLocator();
	    WSWise2 wise2 = service.getWSWise2();
	    String jobid = wise2.runWise2(params,inputs);

	    if (line.hasOption("async")) {
               System.out.println("This is your job id: "+ jobid);
	       System.out.println("Use: java WSWise2Client --status --jobid "+ jobid + " to see the job status");
	    } else {
         getResults(jobid,line);
	    }	
      } else {
