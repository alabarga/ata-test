...
public class WSKalignClient {
	...
	System.out.println("Use: java WSKalignClient [options] --email your@email.com --program PROGNAME --database DBNAME --sequence file");
	System.out.println("Use: java WSKalignClient [options] --email your@email.com --program PROGNAME --database DBNAME --sequence db:entry_id");
	System.out.println("");
	...
	Iterator ii = options.getOptions().iterator();
	 ...
	 System.out.println(" --" + opt.getLongOpt() + " : " + opt.getDescription());
	    ...
	    System.out.println("  Usage: java WSKalignClient [options] --program PROGNAME --database DBNAME --sequence file [--outfile string]");
	    ...
	    System.out.println("  Use: java WSKalignClient [options] --program PROGNAME --database DBNAME --sequence file --async");
	    ...
	    System.out.println("");
	    System.out.println("  When done, WSKalignClient --polljob --jobid string [--outfile string]");
	    System.out.println("");
    ...
    private static int checkStatus(String jobid) throws IOException {
	 try {
	    ...
	    WSKalignService service =  new WSKalignServiceLocator();
	    WSKalign kalign = service.getWSKalign();
	    String result =  kalign.checkStatus(jobid);
	    System.out.println(result);
	    System.err.println("This is your job id: "+ jobid+ " ( " +result+" )");
	    System.err.println("Use: java WSKalignClient --polljob --jobid "+ jobid + " to see the results");	
    ...
    private static int getResults(String jobid, CommandLine line) throws IOException {
	 try {
	    ...
	    if (line.hasOption("outformat")) {
         ...
         byte[] resultbytes = kalign.poll(jobid,format);
	     ...
	     if (line.hasOption("stdout")){ 
 	     ...
 	     WSFile[] results = kalign.getResults(jobid);

	     for (int i=0;i<results.length;i++){
			WSFile file = results[i];
			byte[] resultbytes = kalign.poll(jobid,file.getType());
			String result = new String(resultbytes);		  
...
private static InputParams loadParams(CommandLine line) throws IOException {
 ...
 try {
	   ...
	   "moltype|m=s" => \$params{moltype},
           "gpo|g=f"     => \$params{gpo},
           "gpe|e=f"     => \$params{gpe},
           "tgpe|t=f"    => \$params{tgpe},
           "bonus|b=f"   => \$params{bonus},
	   "help|h"	 => \$help,
    ...
    if (line.hasOption("moltype")) params.setMoltype(line.getOptionValue("moltype")); 	
    if (line.hasOption("gpo")) params.setGpo(line.getOptionValue("gpo"));
...
options.addOption(OptionBuilder.withLongOpt("database")
    ...
    try {
      ...
      if (line.hasOption("sequence")||line.hasOption("dbfetch")||line.hasOption("ebifile")) {
	    ...
	    WSKalignService service =  new WSKalignServiceLocator();
	    WSKalign kalign = service.getWSKalign();
	    String jobid = kalign.runKalign(params,inputs);

	    if (line.hasOption("async")) {
		   System.out.println(jobid); 
           System.err.println("This is your job id: "+ jobid);
	       System.err.println("Use: java WSKalignClient --status --jobid "+ jobid + " to see the job status");
	    } else {
         getResults(jobid,line);
	    }	
      } else {
